...
export class BreakHandler {
    whitelistCheck = new WhitelistCheckHandler();
    toolCheck = new ToolCheckHandler();
    specialRule = new SpecialRuleHandler();
    limitCheck = new LimitCheckHandler();
    durability = new DurabilityHandler();
    drop = new DropHandler();
    /**
     * 处理单个方块的破坏
     * @returns 是否成功破坏
     */
    handle(player, block, registry) {
        try {
            // 再次校验白名单（防止并发修改）
